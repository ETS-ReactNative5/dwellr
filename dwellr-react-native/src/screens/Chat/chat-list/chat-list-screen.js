...
export function ChatListScreen({ navigation, route }) {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity style={styles.addButton} onPress= {() => navigation.navigate('Chat create')}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ChatListItem
              channel={item}
              onPress={() => navigation.navigate('Chatroom', { channelId: item.id, identity: username })}
            />
          )}
